...
On your local machine, use [w3cli] to generate a new a new Ed25519 privagte signing key for CI to use.
...
Set `W3_PRINCIPAL=<the signing key>` in the CI environment. The `w3` commmand will use the value as the signing key to use. see: https://github.com/web3-storage/w3cli?tab=readme-ov-file#w3_principal
...
The [add-to-web3] action is lightweight wrapper around [w3cli]. You create the key and proof as before, and the action configures and runs `w3` in CI for you.
